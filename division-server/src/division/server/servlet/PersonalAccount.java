...
public class PersonalAccount extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    try {
      Command command = Command.valueOf(Client.getData(request).getString("command"));
  ...
  private JSONObject setWritable(HttpServletRequest request) throws Exception {
    PropertyMap comment = PropertyMap.create().setValue("id", Client.getData(request).getInt("id")).setValue("type", Comment.Type.CURRENT);
    ObjectLoader.update(Comment.class, comment);
    return Client.OK(comment);
  }
  
  private JSONObject getComments(HttpServletRequest request) throws Exception {
  ...
  private JSONObject addComment(HttpServletRequest request) throws Exception {
    Integer requestId = Client.getData(request).getInt("id");
    String text = Client.getData(request).getString("text");
  ...
  private JSONObject getRequestData(HttpServletRequest request) throws Exception {
    PropertyMap o = ObjectLoader.getMap(Client.Class("Request"), Client.getData(request).getInt("id"));
  ...
  private JSONObject showRequests(HttpServletRequest request) throws Exception {
  ...
  private JSONObject saveRequest(HttpServletRequest request) throws Exception {
  ...
  private JSONObject getEquipments(HttpServletRequest request) throws Exception {
  ...
  private JSONObject saveCommand(HttpServletRequest request, JSONObject json, Command command) throws Exception {
  ...
  private JSONObject getContracts(HttpServletRequest request) throws RemoteException {
    ...
    if(data != null) {
      ...
      data.stream().forEach(d -> {
        ...
        contract.accumulate("id",        d.get(0));
        contract.accumulate("number",    d.get(1));
        contract.accumulate("name",      d.get(2));
        contract.accumulate("balance",   d.get(4) == null ? BigDecimal.ZERO : d.get(4));
          ...
          for(String p:(String[])d.get(3)) {
            if(!ArrayUtils.contains(ids, Integer.valueOf(p.split(",")[0]))) {
              ...
              processes.add(new JSONObject()
                      .accumulate("id", Integer.valueOf(p.split(",")[0]))
                      .accumulate("name", p.substring(p.indexOf(",")+1)));
            }
          }
        }
        contract.accumulate("processes", processes);
        contracts.add(contract);
      });
      return Client.OK(new JSONObject()
              .accumulate("name", Client.getClientName(request))
              .accumulate("contracts", contracts));
  ...
  private void getAct(HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    if(XMLTemplate != null && !XMLTemplate.equals("")) {
      JSONObject currentData = Client.getData(request);
      Integer year  = currentData.getInt("year");
      Integer kv    = currentData.containsKey("kv") ? currentData.getInt("kv") : null;
      ...
      Client.getList(DBFilter.create("Contract").AND_IN("id", currentData.getJSONArray("contract").toArray(new Integer[0])), 
  ...
  private void getDocument(boolean download, HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    if(currentData.containsKey("ids")) {
      for(Object id:currentData.getJSONArray("ids"))
        ids = (Integer[]) ArrayUtils.add(ids, id);
    }else if(currentData.containsKey("id")) 
      ids = (Integer[]) ArrayUtils.add(ids, currentData.getInt("id"));
  ...
  private JSONObject showDocs(HttpServletRequest request) throws RemoteException {
    Integer[] contracts = new Integer[0];
    JSONObject currentData = Client.getData(request);
    for(Object id:currentData.getJSONArray("contract"))
      contracts = (Integer[]) ArrayUtils.add(contracts, id);
    Integer year  = currentData.getInt("year");
    Integer month = currentData.getInt("month");
    long start,end;
    ...
    if(db != null) {
      ...
      for(List d:db) {
        data.add(new JSONObject()
                .accumulate("id",      d.get(0))
                .accumulate("name",    d.get(1))
                .accumulate("number",  d.get(2))
                .accumulate("date",  Utility.format((Timestamp)d.get(3)))
                .accumulate("templ", d.get(4)));
      }
  ...
  private JSONObject getYears(HttpServletRequest request) throws RemoteException {
    ...
    if(db != null) {
      ...
      db.stream().forEach(d -> {
        data.accumulate("max", d.get(0));
        data.accumulate("min", d.get(1));
      });
      return Client.OK(data);
    }
    return Client.ERROR("Ошибка запроса в БД");
