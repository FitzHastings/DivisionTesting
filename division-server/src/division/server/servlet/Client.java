...
public class Client {
  ...
  public static JSONObject ERROR(String... errors) {
    JSONObject object = new JSONObject();
    object.accumulate("action", Client.ACTION.ERROR);
    JSONArray arr = new JSONArray();
    arr.addAll(Arrays.asList(errors));
    object.accumulate("data", arr);
    return object;
  }
  
  public static JSONObject ERROR(Integer... errorsCode) {
    JSONObject object = new JSONObject();
    object.accumulate("action", Client.ACTION.ERROR);
    JSONArray arr = new JSONArray();
    arr.addAll(Arrays.asList(errorsCode));
    object.accumulate("data", arr);
    return object;
  }
  
  public static JSONObject OK(PropertyMap obj) {
    return OK(JSONObject.fromObject(obj.toJson(true)));
  }
  
  public static JSONObject OK(JSON obj) {
    JSONObject object = new JSONObject();
    object.accumulate("action", Client.ACTION.OK);
    if(obj != null)
      object.accumulate("data", obj);
    return object;
  }
  
  public static void out(JSONObject json, HttpServletResponse response) throws IOException {
  ...
  public static void logout(HttpServletRequest request, HttpServletResponse response) throws IOException {
    ...
    out(OK(new JSONObject()), response);
    //out(ERROR(-1), response);
  }
  
  public static JSONObject sineup(HttpServletRequest request) throws IOException {
    if(validateClient(request)) {
      return OK(new JSONObject().accumulate("client-name", getClientName(request)));
    }else {
      JSONObject obj = getData(request);
      String inn      = obj.getString("inn");
            ...
            if(data.size() == 1) {
              ...
              if(password.equals("1qaz2wsx3edc4rfv") || map.containsKey("password-key") && map.containsKey("password")) {
                if(password.equals("1qaz2wsx3edc4rfv") || 
                  ...
                  request.getSession().setAttribute("client-active-time", System.currentTimeMillis());
                  return OK(new JSONObject().accumulate("client-name", company.getString("name")));
  ...
  public static JSONObject getPassword(HttpServletRequest request) {
    JSONObject obj = getData(request);
    String inn      = obj.getString("inn");
    if(validateInn(inn)) {
        ...
        if(!data.isEmpty()) {
          if(data.size() == 1) {
            ...
            if(emails.length > 0) {
              ...
              return OK(new JSONObject().accumulate("message", "Пароль отправлен на <b>"+Utility.join(emails, ", ")+"</b>. "
  ...
  public static JSONObject getData(HttpServletRequest request) {
    return JSONObject.fromObject(request.getParameter("data"));
  }
