...
public class DocumentDateChooser extends Stage{
  private Scene scene;
  private VBox root;
  ...
  public DocumentDateChooser(Map<Integer,PropertyMap> documents, boolean splitFunction) {
    ...
    if(splitFunction) {
      defSplit.getItems().get(1).fire();
    }
  }
  
  public boolean get(Parent parent) {
    setScene(scene);
    if(parent != null)
  ...
  private void initComponents() {
    ...
    ok.setOnAction((ActionEvent event) -> {
      table.getSourceItems().stream().forEach(it -> {
        ...
        documents.get(it.getValue("id")).setValue("autonumber", it.getValue("number"));
      });
