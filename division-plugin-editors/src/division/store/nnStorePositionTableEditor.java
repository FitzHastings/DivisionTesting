...
public class nnStorePositionTableEditor extends EditorGui {
  ...
  private DivisionTarget storeTarget = null;
  
  public nnStorePositionTableEditor() {
    this(null);
  }
  
  public nnStorePositionTableEditor(Integer[] stories) {
    super(null, null);
  ...
  public void changeSelection(EditorGui editor, Integer[] ids) {
    ...
    if(stories.length == 1) {
      if(storeTarget != null)
        removeTarget(storeTarget);
      addTarget(storeTarget = new DivisionTarget(Store.class, stories[0]) {
        @Override
  ...
  private void initComponents() {
    ...
    table.setDefaultRenderer(BigDecimal.class, new DivisionTableRenderer() {
      ...
      public Component getTableCellRendererComponent(JTable t, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
        StoreRow storeRow = (StoreRow) t.getValueAt(row, 0);
        if(column == getColumnIndex(ColumnName.Депозитарий))
          value = storeRow.getStoreName();
        if(column == getColumnIndex(ColumnName.Наименование))
          value = storeRow.getGroupName();
        if(column == getColumnIndex(ColumnName.ед_изм))
          value = storeRow.getGroupUnit();
        if(column == getColumnIndex(ColumnName.Наличие))
          value = storeRow.getNalichie();
        if(column == getColumnIndex(ColumnName.Резерв))
          value = storeRow.getReserve();
        if(column == getColumnIndex(ColumnName.Доступно))
          value = storeRow.getDostupno();
      ...
      public void mouseClicked(MouseEvent e) {
        ...
        if(e.getClickCount() == 2 && row >= 0 && col >= 0) {
          ...
          if(row >= 0 && col >= columns.size()) {
            ...
            if(factorColumn.getFactorType() == Factor.FactorType.адрес) {
  ...
  private JCheckBox createFactorColumnCheckBox(Integer id, String name, Factor.FactorType factorType, String listValues, boolean productFactor) {
  ...
  /*private void initFactorsMenu() {
    ...
    Vector<Vector> data = ObjectLoader.executeQuery("SELECT "
            + "[Factor(id)], "
            + "[Factor(name)] ,"
            + "[Factor(factorType)], "
            + "[Factor(listValues)] "
    ...
    for(int i=0;i<data.size();i++)
      factorsPanel.add(
              createFactorColumnCheckBox((Integer) data.get(i).get(0), 
                      ...
                      Factor.FactorType.valueOf((String) data.get(i).get(2)), 
          ...
          for(List d:data) {
            ...
            if(groupCheck.isSelected()) {
              ...
              for(Vector row:dataVector) {
                StoreRow storeRow = (StoreRow) row.get(0);
                if(storeRow.containsHashcode((String) d.get(3))) {
                  updateRow = row;
                  break;
                }
              }
              ...
              for(Vector row:dataVector) {
                StoreRow storeRow = (StoreRow) row.get(0);
                if(storeRow.containsId((Integer) d.get(0))) {
                  updateRow = row;
                  break;
                }
              }
            ...
            if(updateRow == null) {// не нашли
              ...
              storeRow.add(
                      ...
                      (String)      d.get(3), 
                      (Integer)     d.get(1), 
                      (String)      d.get(idColumns.size()+getColumnIndex(ColumnName.Депозитарий)), 
                      (Integer)     d.get(2), 
                      (String)      d.get(idColumns.size()+getColumnIndex(ColumnName.Наименование)), 
                      (String)      d.get(idColumns.size()+getColumnIndex(ColumnName.ед_изм)), 
                      (BigDecimal)  d.get(idColumns.size()+getColumnIndex(ColumnName.Наличие)), 
