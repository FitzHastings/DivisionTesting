...
public class MoneyPosition extends EditorGui implements ClientMainListener {
  ...
  private Integer[] cfc      = new Integer[0];
  ...
  private DefaultTableCellRenderer renderer = new DefaultTableCellRenderer() {
    ...
    public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
      ...
      TreePath path = MoneyPosition.this.table.getTree().getPathForRow(row);
      if(path != null) {
  ...
  public MoneyPosition() {
    ...
    try {
      ...
      if(!data.isEmpty())
        cfc = new Integer[]{(Integer) data.get(0).get(0)};
    }catch(Exception ex) {
  ...
  private void initComponents() {
    ...
    getToolPanel().setLayout(new FlowLayout(FlowLayout.TRAILING, 10, 5));
    getToolPanel().add(new JLabel("с"));
    getToolPanel().add(start);
    getToolPanel().add(new JLabel("по"));
    getToolPanel().add(end);
    getToolPanel().add(go);
    getToolPanel().add(graf);
  ...
  public void profitLine(final LineChart chart, final String name, final LocalDate fstart, final LocalDate end, final int step) {
    ...
    p.submit(() -> {
      ...
      while(start.isBefore(end) || start.isEqual(end)) {
        ...
        switch(step) {
          case Calendar.MONTH:
            profitY = calcProfit(start.withDayOfMonth(1), start.withDayOfMonth(start.lengthOfMonth())).setScale(2, RoundingMode.HALF_UP);
            ...
            break;
          case Calendar.YEAR:
            profitY = calcProfit(start.withDayOfYear(1), start.withDayOfYear(start.lengthOfYear())).setScale(2, RoundingMode.HALF_UP);
          ...
          case Calendar.DAY_OF_MONTH:
            profitY = calcProfit(start, start.plusDays(1)).setScale(2, RoundingMode.HALF_UP);
    ...
    for(Integer process:processes) {
      ...
      p.submit(() -> {
        ...
        while(start.isBefore(end) || start.isEqual(end)) {
          ...
          switch(step) {
            case Calendar.MONTH:
              Y   = calcPlus(start.withDayOfMonth(1), start.withDayOfMonth(start.lengthOfMonth()), process == -1 ? null : process == 0 ? processes.subList(2, processes.size()).toArray(new Integer[0]) : new Integer[]{process}).setScale(2, RoundingMode.HALF_UP);
              final String monthLabel = start.withDayOfMonth(start.lengthOfMonth()).getMonth().getDisplayName(TextStyle.FULL_STANDALONE, Locale.getDefault())+" "+start.withDayOfMonth(start.lengthOfMonth()).format(DateTimeFormatter.ofPattern("yyyy"));
              Platform.runLater(() -> data.getData().add(new XYChart.Data(monthLabel, Y)));
              start = start.plusMonths(1);
              break;
            case Calendar.YEAR:
              Y = calcPlus(start.withDayOfYear(1), start.withDayOfYear(start.lengthOfYear()), process == -1 ? null : process == 0 ? processes.subList(2, processes.size()).toArray(new Integer[0]) : new Integer[]{process}).setScale(2, RoundingMode.HALF_UP);
              final String yearLabel = start.withDayOfMonth(start.lengthOfYear()).format(DateTimeFormatter.ofPattern("yyyy"));
              Platform.runLater(() -> data.getData().add(new XYChart.Data(yearLabel, Y)));
              start = start.plusYears(1);
              break;
            case Calendar.DAY_OF_MONTH:
              Y = calcPlus(start, start.plusDays(1), process == -1 ? null : process == 0 ? processes.subList(2, processes.size()).toArray(new Integer[0]) : new Integer[]{process}).setScale(2, RoundingMode.HALF_UP);
              final String dateLabel = start.format(DateTimeFormatter.ofPattern("dd.MM.yyyy"));
  ...
  public void minusLine(final LineChart chart, final String name, final LocalDate fstart, final LocalDate end, final int step) {
    ...
    p.submit(() -> {
      ...
      while(start.isBefore(end) || start.isEqual(end)) {
        ...
        switch(step) {
          case Calendar.MONTH:
            Y   = calcMinus(start.withDayOfMonth(1), start.withDayOfMonth(start.lengthOfMonth())).setScale(2, RoundingMode.HALF_UP);
            final String monthLabel = start.withDayOfMonth(start.lengthOfMonth()).getMonth().getDisplayName(TextStyle.FULL_STANDALONE, Locale.getDefault())+" "+start.withDayOfMonth(start.lengthOfMonth()).format(DateTimeFormatter.ofPattern("yyyy"));
            Platform.runLater(() -> data.getData().add(new XYChart.Data(monthLabel, Y)));
            start = start.plusMonths(1);
            break;
          case Calendar.YEAR:
            Y = calcMinus(start.withDayOfYear(1), start.withDayOfYear(start.lengthOfYear())).setScale(2, RoundingMode.HALF_UP);
            final String yearLabel = start.withDayOfMonth(start.lengthOfYear()).format(DateTimeFormatter.ofPattern("yyyy"));
            Platform.runLater(() -> data.getData().add(new XYChart.Data(yearLabel, Y)));
            start = start.plusYears(1);
            break;
          case Calendar.DAY_OF_MONTH:
            Y = calcMinus(start, start.plusDays(1)).setScale(2, RoundingMode.HALF_UP);
            final String dateLabel = start.format(DateTimeFormatter.ofPattern("dd.MM.yyyy"));
            Platform.runLater(() -> data.getData().add(new XYChart.Data(dateLabel, Y)));
            start = start.plusDays(1);
            break;
  ...
  private BigDecimal calcPlus(LocalDate start, LocalDate end, Integer[] processes) {
    ...
    BigDecimal d = (BigDecimal)ObjectLoader.executeQuery("SELECT NULLTOZERO((SELECT SUM([Deal(costWihoutNds)]) FROM [Deal] WHERE "
            ...
            + "[Deal(finishDate)] BETWEEN ? AND ? AND [Deal(dealEndDate)] <= ?))", 
            params.toArray()).get(0).get(0);
    return d;
  }
  
  private BigDecimal calcMinus(LocalDate start, LocalDate end) {
    BigDecimal d = (BigDecimal)ObjectLoader.executeQuery("SELECT NULLTOZERO((SELECT SUM([Deal(costWihoutNds)]) FROM [Deal] WHERE "
            + "[Deal(customerCfc)]=ANY(?) AND "
            + "(SELECT COUNT(id) FROM [DealPosition] WHERE [DealPosition(deal)] = [Deal(id)] AND [DealPosition(dispatchId)] IS NULL AND tmp=false) = 0 AND"
            + "(SELECT SUM([DealPosition(customProductCost)] * [DealPosition(amount)]) FROM [DealPosition] WHERE [DealPosition(deal)] = [Deal(id)]) = (SELECT SUM([DealPayment(amount)]) FROM [DealPayment] WHERE [DealPayment(deal)] = [Deal(id)] AND tmp = false AND type='CURRENT') AND "
            + "[Deal(finishDate)] BETWEEN ? AND ? AND [Deal(dealEndDate)] <= ?))", 
  ...
  private BigDecimal calcProfit(LocalDate start, LocalDate end) {
    BigDecimal d = (BigDecimal)ObjectLoader.executeQuery("SELECT NULLTOZERO((SELECT SUM([Deal(costWihoutNds)]) FROM [Deal] WHERE "
            + "[Deal(sellerCfc)]=ANY(?) AND "
            + "(SELECT COUNT(id) FROM [DealPosition] WHERE [DealPosition(deal)] = [Deal(id)] AND [DealPosition(dispatchId)] IS NULL AND tmp=false) = 0 AND"
            + "(SELECT SUM([DealPosition(customProductCost)] * [DealPosition(amount)]) FROM [DealPosition] WHERE [DealPosition(deal)] = [Deal(id)]) = (SELECT SUM([DealPayment(amount)]) FROM [DealPayment] WHERE [DealPayment(deal)] = [Deal(id)] AND tmp = false AND type='CURRENT') AND "
  ...
  class InitDataTask implements Runnable {
    ...
    public void run() {
      ...
      try {
        if(isShutdoun() || cfc == null || cfc.length == 0)
          return;
        
        String[] n = new String[0];
        java.util.List<java.util.List> cfcData = ObjectLoader.getData(CFC.class, cfc, new String[]{"name"});
        for(java.util.List d:cfcData)
          n = (String[]) ArrayUtils.add(n, d.get(0));
        cfcName.setText(String.join(", ", n));
        ...
        dateFilter.clear()
                .AND_TIMESTAMP_BETWEEN("positionDate", new Timestamp(start.getDate().getTime()), new Timestamp(c.getTimeInMillis()));
                //.AND_TIMESTAMP_BEFORE_OR_EQUAL("dealEndDate", new Timestamp(c.getTimeInMillis()));

        sideFilter.clear().AND_IN("sellerCfc", cfc);
        if(companys != null && companys.length > 0)
          sideFilter.AND_IN("sellerCompany", companys);
        sideFilter.OR_IN("customerCfc", cfc);
        if(companys != null && companys.length > 0)
        ...
        SwingUtilities.invokeLater(() -> {
          try {
            ...
            for(java.util.List d:data) {
              if(isShutdoun())
                return;
              seller = ArrayUtils.contains(cfc, d.get(3));
              if(seller && ArrayUtils.contains(cfc, d.get(13)))
                continue;
              parentNode = null;
  ...
  public class MoneyPositionNode extends SwingNode {
    ...
    public MoneyPositionNode(Integer dealId, Integer processId, String processName, String dohod, String rashod, java.sql.Date lastDispatchDate, java.sql.Date lastPayDate, java.sql.Date finishDate, String CFCName, String companyName) {
      super(Utility.format(finishDate));
      this.dealId      = dealId;
      this.processId   = processId;
      this.dohod       = dohod;
      this.rashod      = rashod;
