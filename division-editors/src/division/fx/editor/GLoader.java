...
public class GLoader {
  ...
  public static void load(String name, Object... storeControls) {
    try {
          ...
          if(doc != null) {
            for(Node control:validateStoreControls(storeControls)) {
              ...
              if(node != null) {
                ...
                if(control instanceof FXTableEditor)
                  control = ((FXTableEditor)control).getTable();
      ...
      ex.printStackTrace();
      Logger.getLogger(GLoader.class).error(ex);
    }
  }
  ...
  public static ObservableList<Node> validateStoreControls(Object... storeControls) {
    ...
    for(Object n:storeControls) {
      if(n instanceof Storable)
        list.addAll(validateStoreControls(((Storable)n).storeControls()));
      else if(n instanceof List)
        list.addAll(validateStoreControls(((List)n).toArray()));
      else if(n instanceof Node)
        list.add((Node)n);
    }
    list.stream().forEach(node -> node.setId(node.getId() == null ? getId(node) : node.getId()));
    return list;
  }
